...
using DaRQ.Compiler.Generator;
using DaRQ.Compiler.Assembler;

namespace DaRQ.JavaScriptCompiler
{
    public class Generator : DaRQ.Compiler.Generator.Generator
    {
        protected int indention = 0;

        public IList<string> GenerateArray(IEnumerable<Node> nodes)
        {
            var walker = new DaRQ.Compiler.Assembler.NodeWalker(nodes.ToArray());
            return Generate(walker).ToList();
        }
        protected override string GenerateRuleCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
        ...
        protected string GenerateCodeLine(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
            if (node.Is(NodeType.Block)) return GenerateCode(node, walker) ?? "";
            return GenerateCode(node, walker) ?? "";
        }

        protected override string GenerateProcedureCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
            return node.Token.Value + " " + string.Join(" ", GenerateArray(node.Children));
        }

        protected override string GenerateComputeCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
            return " " + node.Token.Value + " " + string.Join(" ", GenerateArray(node.Children));
        }

        protected override string GeneratePlainCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
            if (node.Count > 0)
        ...
        protected override string GenerateDefineCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
        ...
        protected override string GenerateCallCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
            if (node.Token.Is(TokenType.FunctionKeyword))
                return $"{node.Token.Value}({string.Join(", ", GenerateArray(node.Children))})";
            if (node.Count > 0)
        ...
        protected override string GenerateBlockCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
            {
        ...
        protected override string GenerateHelperCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
            if (node.Count > 0)
        ...
        protected override string GenerateProgramCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
            if (Compiler != null) return Transform(node, Compiler) as string;
            return null;
        }

        protected override string GenerateUnknownCode(Node node, DaRQ.Compiler.Assembler.NodeWalker walker)
        {
